...
export class UploadsValidationPipe implements PipeTransform {
  transform(
    ...
    const allowedDocsTypes = ["application/pdf","image/jpeg", "image/png"];
    ...
    for (const [field, files] of Object.entries(uploads)) {
      ...
      if (field === "avatar" && files.length > 1) {
        throw new BadRequestException("avatar max 1 file");
      }
      ...
      files.forEach((file, index) => {
        if (file.size > MAX_SIZE) {
          throw new BadRequestException(`${field}[${index}] exceeds 5MB`);
        }

        if (field === "beforeWarImage" || field === "afterWarImage"||field === "ownershipDocuments") {
          if (!allowedDocsTypes.includes(file.mimetype)) {
